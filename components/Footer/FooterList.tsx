...
const FooterList: React.FC<Props> = ({ footerData }) => {
  ...
  return (
    <article className='space-y-5'>
      <h4 className='text-xs font-semibold tracking-wide text-gray-900 uppercase'>{header}</h4>
      <ul className='space-y-4'>
        {routeLinks.map((routeLink: RouteLink, index: number) => (
          <li
            className='hover:text-gray-900 transition-colors duration-200 tracking-tight font-medium'
            key={index}
          >
            {outerLink ? (
